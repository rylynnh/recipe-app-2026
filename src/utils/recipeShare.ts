...
export async function createRecipeShareImage(recipe: Recipe): Promise<Blob> {
  ...
  canvas.height = Math.max(1400, 610 + 260 + titleLines.length * 58 + ingredientLines.length * 48 + stepLines.length * 46 + recipe.steps.length * 38);
  ...
  let y = drawCover(ctx, cover, 0) + 58;
  if (cover && 'close' in cover) cover.close();
  ctx.fillStyle = '#A87945'; ctx.font = '20px "Helvetica Neue", Arial, sans-serif'; ctx.letterSpacing = '3px'; ctx.fillText('MISE  /  RECIPE', PADDING, y); ctx.letterSpacing = '0px'; y += 52;
  ctx.fillStyle = '#242321'; ctx.font = '48px "Songti SC", "STSong", serif';
  for (const line of titleLines) { ctx.fillText(line, PADDING, y); y += 58; }
  y += 18; ctx.fillStyle = '#77726B'; ctx.font = '24px "PingFang SC", "Microsoft YaHei", sans-serif'; ctx.fillText(tagText, PADDING, y); y += 50;
